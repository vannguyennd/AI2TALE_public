    ...
    if os.path.exists(model_path):
                    ...
                    for i_selected_text in x_val_selected_text_process[idx].split("\n"):
                        if i_selected_text.strip() == "":
                            ...
                            if check_in_dictionary(i_selected_text.strip(), english_words) == 0:
                                ...
                                sum, c_scar, c_con, c_rec, c_aut, c_soc, c_lik, c_sec, check_important = check_cog_diversity(
                            ...
                            if check_in_dictionary(i_selected_text.strip(), english_words) == 0:
                                ...
                                if sum == 0:
                                    ...
                                    if check_important:
                                        total_important += 1
                                    num_wr_count_cog += 1
                                    sum_wr_count_cog += sum
                                    important_sentences += 1

        if num_co_count_cog == 0:
            acoc = 0
        else:
            acoc = (1.0*sum_co_count_cog)/(num_co_count_cog)
        if num_wr_count_cog == 0:
            awrc = 0
        else:
            awrc = (1.0*sum_wr_count_cog)/(num_wr_count_cog)
        if num_co_count_cog + num_wr_count_cog == 0:
            tacowrc = 0
        else:
            tacowrc = (1.0*(sum_co_count_cog+sum_wr_count_cog)) / (num_co_count_cog+num_wr_count_cog)

        print("pro_co_cblank: " + str(pro_co_cblank))
        print("pro_co_cdump: " + str(pro_co_cdump))
        print("pro_co_cnot_cog: " + str(pro_co_cnot_cog))
        print("ave_co_count_cog(^): " + str(acoc))

        print("pro_wr_cblank: " + str(pro_wr_cblank))
        print("pro_wr_cdump: " + str(pro_wr_cdump))
        print("pro_wr_cnot_cog: " + str(pro_wr_cnot_cog))
        print("ave_wr_count_cog(^): " + str(awrc))

        print("t_ave_cowr_count_cog(^): " + str(tacowrc))
        ...
        if total_sentences == 0:
            ...
            ip = 0
             ...
             ip = (1.0*total_important)/total_sentences
        
        print("total_sentences: " + str(total_sentences))
        print("important_sentences(^): " + str(important_sentences))
        print("true positive(^): " + str(tp))
        
        print("important percent(s)(^): " + str(ip))

        file_results.write('pro_co_cblank: {}; pro_co_cdump: {}; pro_wr_cblank: {}; pro_wr_cdump: {}; \
                           pro_wr_cblank: {}; pro_wr_cdump: {};\n ave_co_count_cog(^): {}; \
                           ave_wr_count_cog(^): {};\n t_ave_cowr_count_cog(^): {}; \
                           ts: {}; is(^): {}; tp(^): {}; ip(^): {}. \
                           \n\n'.format(pro_co_cblank, pro_co_cdump,
                                        pro_wr_cblank, pro_wr_cdump, pro_co_cnot_cog, pro_wr_cnot_cog, 
                                        acoc, awrc, tacowrc, total_sentences, important_sentences, tp, ip))

        data = {'x_val_text': x_val_text,
                'x_val_selected_text_process': x_val_selected_text_process,
                'pred_val': pred_val,
                'y_val': y_val}
        data_df = pd.DataFrame(data)
        data_df.to_csv(model_path + "x_selected_val_text_get_results_" + str(args.k) + ".csv")
